...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor,DynaSLAM::SegmentDynObject* pMaskRCNN,string inFiledir,string outFiledir):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0),mpMaskRCNN(pMaskRCNN),
    msinFiledir(inFiledir),msoutFiledir(outFiledir),
    mtORB(0.0), mtinit(0.0), mtNormalTrack(0.0), mtmask(0.0),minitcount(0), mnormalcount(0), mOKcount(0),mmaskcount(0)
{
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im,const double &timestamp,const string picname)
{
    printf("mtORB %.4lf,mtNormalTrack %.4f,mtinit %.4f,minitcount %.4d,mnormalcount %.4d,mOKcount %4d\n", mtORB, mtNormalTrack, mtinit, minitcount, mnormalcount, mOKcount);
    printf("mtmask %.4f,mmaskcount %4d\n",mtmask,mmaskcount);
    mImGray = im;
    cv::Mat imMask = mMask;
    ...
    {
        mCurrentFrame = Frame(mImGray,imMask,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,picname);
    }
    else
        mCurrentFrame = Frame(mImGray,imMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,picname);
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK)
            ...
            {
                std::cout<<"LOST"<<mCurrentFrame.mspicname<<"\n";
                mState=LOST;
    ...
    if(mState!=OK)
    {
        miOK=0;
        mbupdateMask=false;
        mMask=cv::Mat::ones(405,720,CV_8U);
        //mState=OK;
    }
	// std::cout<<"miOK "<<miOK<<"\n";
    }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    printf("KF: \n",mCurrentFrame.mTimeStamp);
    std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
    double tmask = std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1).count();
    mtmask += tmask;
    mmaskcount++;
...
{
    if(!mbupdateMask||msoutFiledir=="no_save")
        return;
    string image_path=msinFiledir+'/'+mCurrentFrame.mspicname;
    cv::Mat mask = cv::Mat::ones(405,720,CV_8U);
    cv::Mat maskRCNN=mpMaskRCNN->GetSegmentation(image_path,msoutFiledir,mCurrentFrame.mspicname);
    cv::Mat maskRCNNdil = maskRCNN.clone();
    cv::dilate(maskRCNN,maskRCNNdil, mkernel);
    mMask = mask - maskRCNNdil;
}
