...
Geometry::Geometry()
{
    vAllPixels = cv::Mat(720*405,2,CV_32F);
    int m(0);
    for (int i(0); i < 720; i++){
        for (int j(0); j < 405; j++){
...
cv::Mat Geometry::DepthRegionGrowing(const vector<DynKeyPoint> &vDynPoints,const cv::Mat &imDepth){
    ...
    cv::Mat maskG = cv::Mat::zeros(405,720,CV_32F);
    ...
    cv::Mat _maskG = cv::Mat::ones(405,720,CV_8U);
    maskG = _maskG - maskG;
...
void Geometry::FillRGBD(const ORB_SLAM2::Frame &currentFrame,cv::Mat &mask,cv::Mat &imGray,cv::Mat &imDepth){
    ...
    for (int i(0); i < mDB.mNumElem; i++){
        ...
        cv::Mat vPixels(720*405,2,CV_32F);
        cv::Mat mDepth(720*405,1,CV_32F);

        int n(0);
        for (int j(0); j < 720*405; j++){
