...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer,string inFiledir,string outFiledir):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
        mbDeactivateLocalizationMode(false),msinFiledir(inFiledir),msoutFiledir(outFiledir)
{
    cout<<"inFiledir"<<msinFiledir<<"\n";
    cout<<"outFiledir"<<msoutFiledir<<"\n";
    cout << "Loading Mask R-CNN. This could take a while..." << endl;
    mpMaskNet = new DynaSLAM::SegmentDynObject();
    cout << "Mask R-CNN loaded!" << endl;
    // Output welcome message
    cout << endl <<
    "DeepDynaSLAM Copyright (C) 2020 Ming Guo, Shanghai Jiaotong University." << endl << endl;
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor,mpMaskNet,inFiledir,outFiledir);
    ...
    char IsPureLocalization='n';
    char IsLoadMap='n';

    #ifndef NDEBUG
        std::cout<<"not run pure localization\nload the map\n";
    #else
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp,string picname)
{
    if(mSensor!=MONOCULAR)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,timestamp,picname);
