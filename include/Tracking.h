...
public:
    Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Map* pMap,
             KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor,DynaSLAM::SegmentDynObject* pMaskRCNN,string inFiledir,string outFiledir);
    ...
    cv::Mat GrabImageMonocular(const cv::Mat &im,const double &timestamp,const string picname);
...
public:
    ...
    void Reset();
    DynaSLAM::SegmentDynObject* mpMaskRCNN;
    string msinFiledir;
    string msoutFiledir;
    cv::Mat mMask=cv::Mat::ones(405,720,CV_8U);
...
protected:
    ...
    cv::Mat mkernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                        cv::Point( dilation_size, dilation_size ) );
    bool mbupdateMask=true;
    int miOK=0;

    //For time record
    double mtORB;
    double mtNormalTrack;
    double mtinit;
    double mtmask;
